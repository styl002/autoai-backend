...
function buildUrl(f) {
  const slug = [f.make, f.model].filter(Boolean)
    .map(s => s.toLowerCase().replace(/\s+/g,"-").replace(/ä/g,"ae").replace(/ö/g,"oe").replace(/ü/g,"ue"))
    .join("/");
  ...
  try {
    ...
    return await page.evaluate(() => {
      ...
      return [...document.querySelectorAll('article[data-item-name="result-item"]')].slice(0, 20).map((card, i) => {
        ...
        return {
          id: `as24-${i}-${Date.now()}`, source: "AutoScout24",
